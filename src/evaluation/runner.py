...
def run_evaluation(
    ...
    for sample in tqdm(
        ...
        try:
            ...
            # Log the error but don't crash the run — record as a failed sample
            error_count += 1
            logger.warning(
                f"Error on {sample.question_id} ({benchmark.name}/{condition.name}): "
                f"{type(e).__name__}: {e}"
            )
            result = {
                "question_id": sample.question_id,
    ...
    if error_count > 0:
        logger.warning(
            f"{error_count} errors in {benchmark.name}/{condition.name} "
            f"(recorded as incorrect, filter with 'error' field in JSONL)"
        )
    ...
    metrics["num_errors"] = error_count
